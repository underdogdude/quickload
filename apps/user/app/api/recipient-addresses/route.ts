...
export async function GET() {
  try {
    ...
    const rows = await db
      ...
      .where(eq(recipientAddresses.userId, session.userId))
      .orderBy(desc(recipientAddresses.isPrimary), desc(recipientAddresses.createdAt));
    return jsonWithCache(
      { ok: true, data: rows.map(serializeRecipientAddress) },
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, data: serializeRecipientAddress(created) });
  } catch (e) {
    if (e instanceof Response) return e;
    const msg = e instanceof Error ? e.message : "Error";
    if (msg.includes("recipient_addresses") || msg.includes("does not exist")) {
      return NextResponse.json(
        { ok: false, error: "Database table missing. Run sql/20260214_recipient_addresses.sql on your database." },
        { status: 503 },
      );
    }
    return NextResponse.json({ ok: false, error: msg }, { status: 500 });

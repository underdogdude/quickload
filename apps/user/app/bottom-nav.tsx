...
function hideBottomNav(pathname: string) {
  ...
  const isPickupRequests =
    pathname === "/pickup/requests" ||
    pathname.startsWith("/pickup/requests/");
  if (pathname.startsWith("/pickup") && !isPickupRequests) return true;
          ...
          if (item.center) {
            return (
              <SendLink key={item.href} href={item.href} className="flex flex-col items-center -mt-7">
                <span className="inline-flex h-[62px] w-[62px] items-center justify-center rounded-full bg-gradient-to-br from-[#0802b8] to-[#00B7FF] p-[3px] shadow-lg">

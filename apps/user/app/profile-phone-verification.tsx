...
function VerifyPhoneInner({ backHref, successHref, successMessage }: PhoneVerificationProps) {
  ...
  const submitVerify = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setTimeout(() => {
        navigateAfterAuth(router, successHref, { hard: true });
      }, 700);
      return true;
    } catch {
          ...
          <form onSubmit={(e) => void handleVerify(e)} className="space-y-4">
              ...
              <div className="flex gap-3">
                {digits.map((digit, index) => (
                  <input
                    ...
                    onPaste={handlePaste}
                    className={`aspect-square min-w-0 flex-1 rounded-lg border text-center text-2xl font-semibold outline-none transition focus:border-[#0802b8] disabled:bg-slate-50 disabled:opacity-60 ${
                      verifying ? "border-[#0802b8]/40" : "border-slate-300"
                    }`}
              ...
              <div className="mt-3 text-right">
                {sending ? (
                  ...
                  <button
                    ...
                    disabled={!canResend}
                    className="inline-flex items-center gap-1 text-xs text-[#0802b8] underline underline-offset-2 disabled:opacity-50"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                type="submit"
                disabled={!canSubmit}
                className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-[#0802b8] px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60"
              >
                {verifying ? (
                  <>
                    <span
...
export function PhoneVerificationPage(props: PhoneVerificationProps) {
  return (
    <Suspense
      fallback={
